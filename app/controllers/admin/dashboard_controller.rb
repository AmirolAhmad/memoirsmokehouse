class Admin::DashboardController < AdminController
  def index
    ...
    @pending_orders = Order.where('status != ?', 3)
    @total_weekly_paid_orders = Order.where(created_at: date_beginning_this_week..Time.zone.now).sum(:total_price)
    @total_monthly_paid_orders = Order.where(created_at: Time.zone.now.beginning_of_month..Time.zone.now.end_of_month).sum(:total_price)
